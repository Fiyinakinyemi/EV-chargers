...
if nrel_response.status_code == 200:
    ...
    and station.get('state', '') == 'CA' 
    and station.get('zip') is not None 
    and (station.get('zip', '').startswith('900') 
         or station.get('zip', '').startswith('913') 
         or station.get('zip', '').startswith('914'))
]
        data_lines.append(f"Total Electric Charging Stations in LA: {len(electric_stations)}")
    ...
    try:
        ...
        file_path = os.path.join(script_directory, 'la_ev_stations.txt')
