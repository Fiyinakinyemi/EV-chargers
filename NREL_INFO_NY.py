...
if nrel_response.status_code == 200:
    ...
    if station.get('fuel_type_code', '') == 'ELEC' 
    and station.get('state', '') == 'NY' 
    and station.get('zip') is not None 
    and (station.get('zip', '').startswith('100') 
         or station.get('zip', '').startswith('112')
         or station.get('zip', '').startswith('113') 
         or station.get('zip', '').startswith('114') 
         or station.get('zip', '').startswith('104')
         or station.get('zip', '').startswith('103')  
         or station.get('zip', '').startswith('111'))
]
        data_lines.append(f"Total Electric Charging Stations in New York City: {len(electric_stations)}")
    ...
    try:
        ...
        file_path = os.path.join(script_directory, 'ny_ev_stations.txt')
